...
class VoteDetailsAction extends CommonAction{
	...
	function index(){
		if($this->check_is_admin()){
			...
			if(isset($_GET['did']) && !empty($_GET['did'])){
				...
				$dinfos = $VoteCommodity->find($_GET['did']);
				...
				if(isset($_POST['search'])){
					if(!empty($_POST['key'])){
						...
						$count = $VoteDetails->where($datas)->count();
						$page = new page($count,100);
						$list = $VoteDetails->relation(true)->where($datas)->limit($page->setlimit())->order('sort')->select();
					}else{
						$this->redirect('/VoteDetails/index');
					}
				}else{
					$count = $VoteDetails->count();
					$page = new page($count,100);
					$list = $VoteDetails->relation(true)->where($datas)->limit($page->setlimit())->order('sort')->select();
				}
	...
	function ajaxinsert(){
		...
		if($data = $VoteDetails->create()){
			if($VoteDetails->add()){
				if(!empty($_POST['image'])){
					$tmpdir = './Public/Content/tmp/';
					$srcdir = './Public/Content/VoteDetails/';
			...
			$this->ajaxReturn(0,$VoteDetails->getError(),0);
		}
	}
	...
	function ajaxedit(){
		...
		if(isset($_POST['id'])){
			$VoteDetails = D('VoteDetails');
			if($data = $VoteDetails->create()){
				$orginfo = $VoteDetails->where('id='.$_POST['id'])->find();
				if($this->checkData($orginfo,$data)){
					$VoteDetails->save($data);
	...
	function ajaxdel(){
		if(!empty($_POST['deleteid'])){
			...
			for($i=0; $i<count($_POST['deleteid']); $i++){
				$data = $VoteDetails->relation(true)->find($_POST['deleteid'][$i]);
				$srcdir = './Public/Content/VoteDetails/';
				unlink($srcdir.$data['image']);
				unlink($srcdir.'thumb_'.$data['image']);
				$num = $VoteDetails->delete($_POST['deleteid'][$i]);
			}
			if($num>0){
