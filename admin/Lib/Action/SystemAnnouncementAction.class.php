...
class SystemAnnouncementAction extends CommonAction{
	...
	function index(){
		if($this->check_is_admin()){
			...
			$SystemAnnouncement = D('SystemAnnouncement');
			...
			if(isset($_POST['search'])){
				if(!empty($_POST['key'])){
					...
					$count = $SystemAnnouncement->where($datas)->count();
					$page = new page($count,15);
					$list = $SystemAnnouncement->relation(true)->where($datas)->limit($page->setlimit())->order('sort,create_date desc')->select();
				}else{
					$this->redirect('/SystemAnnouncement/index');
				}
			}else{
				$count = $SystemAnnouncement->count();
				$page = new page($count,15);
				$list = $SystemAnnouncement->relation(true)->limit($page->setlimit())->order('sort,create_date desc')->select();
			}
	...
	function edit(){
		if($this->check_is_admin()){
			if(isset($_GET['id'])){
				...
				$data = $SystemAnnouncement->find($_GET['id']);
	...
	function ajaxinsert(){
		...
		if($data = $SystemAnnouncement->create()){
			if($SystemAnnouncement->add()){
				if(!empty($_POST['image'])){
					$tmpdir = './Public/Content/tmp/';
					$srcdir = './Public/Content/SystemAnnouncement/';
			...
			$this->ajaxReturn(0,$SystemAnnouncement->getError(),0);
		}
	}
	...
	function ajaxedit(){
		...
		if(isset($_POST['id'])){
			$SystemAnnouncement = D('SystemAnnouncement');
			if($data = $SystemAnnouncement->create()){
				$orginfo = $SystemAnnouncement->where('id='.$_POST['id'])->find();
				if($this->checkData($orginfo,$data)){
					$SystemAnnouncement->save($data);
	...
	function ajaxdel(){
		if(!empty($_POST['deleteid'])){
			...
			for($i=0; $i<count($_POST['deleteid']); $i++){
				$data = $SystemAnnouncement->relation(true)->find($_POST['deleteid'][$i]);
				if(empty($data['FolkCustomProvinces'])){
					$srcdir = './Public/Content/SystemAnnouncement/';
					unlink($srcdir.$data['image']);
					unlink($srcdir.'thumb_'.$data['image']);
					$num = $SystemAnnouncement->delete($_POST['deleteid'][$i]);
				}else{
