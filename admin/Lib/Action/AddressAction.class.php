...
class AddressAction extends CommonAction{
	...
	function index(){
		if($this->check_is_admin()){
			...
			if(isset($_GET['pid'])){
				...
				$datas['level'] = C('address_level.province_level');
			}
	...
	function city(){
		if($this->check_is_admin()){
			...
			if(isset($_GET['pid'])){
				...
				$datas['level'] = C('address_level.city_level');
			}

			$clist = $MODULE_NAME->where('level='.C('address_level.province_level'))->order('sort')->select();
	...
	function county(){
		if($this->check_is_admin()){
			...
			if(isset($_GET['pid'])){
				...
				$datas['level'] = C('address_level.county_level');
			}

			$clist = $MODULE_NAME->where('level='.C('address_level.city_level'))->order('sort')->select();
			$this->assign('clist',$clist);
	...
	function add(){
		if($this->check_is_admin()){
			...
			$st = $MODULE_NAME->where('start=1')->find();
			$this->assign('st',$st);
			if($st){
				$stt = $MODULE_NAME->where('id='.$st['pid'])->find();
				$this->assign('stt',$stt);
			}
		...
		if(isset($_POST['id'])){
			...
			if($data = $MODULE_NAME->create()){
				...
				if($this->checkData($orginfo,$data)){
					if($data['start']==1){
						$MODULE_NAME->where('id<>'.$orginfo['id'])->setField('start',0);
					}
