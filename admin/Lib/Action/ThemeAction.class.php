...
class ThemeAction extends CommonAction{
	...
	function index(){
		...
		if(isset($_POST['search'])){
			if(!empty($_POST['key'])){
				...
				$count = $Theme->where($datas)->count();
				$page = new page($count,15);
				$list = $Theme->relation(true)->where($datas)->limit($page->setlimit())->order('sort')->select();
			}else{
			...
			$count = $Theme->count();
			$page = new page($count,15);
			$list = $Theme->relation(true)->limit($page->setlimit())->order('sort')->select();
		}
	...
	function ajaxinsert(){
		...
		if($data = $Theme->create()){
			...
			$this->ajaxReturn(0,$Theme->getError(),0);
		}
	}
	...
	function ajaxdel(){
		if(!empty($_POST['deleteid'])){
			...
			for($i=0; $i<count($_POST['deleteid']); $i++){
				$info = $Theme->relation(true)->find($_POST['deleteid'][$i]);
				if(count($info['Contribute'])>0){
					$this->ajaxReturn(0,'该分类下还有内容,不能被删除！',0);
				}else{
					$num = $Theme->delete($_POST['deleteid'][$i]);
				}
			}
			if($num>0){
				...
				if(isset($_POST['publish'])){
					$Theme = D('Theme');
						if($_POST['publish']){
							...
							$Theme->where('id='.$_POST['id'])->setField($data);
