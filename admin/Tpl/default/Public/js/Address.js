$(function(){
    ...
    $("#Address-add-edit [title]").live('hover',function(){
        $(this).tooltip({
	...
	//AJAX删除
	$("[name=Address-del]").live('submit',function(){
		...
		if(data==''){
            ...
            var submit = function (v, h, f) {
                if (v == 'ok'){
                    jBox.tip("正在处理...", 'loading');
                    $("[name=Address-del]").find("[type=submit]").attr('disabled','disabled');
                    window.setTimeout(function () {
                        $.ajax({
                            url: define_app_url+'/Address/ajaxdel',
                            data: data,
                            type:'POST',
                            success: function(data){
                                var msg = JSON.parse(data);
                                if(msg.status==1){
                                    ...
                                    jBox.tip(msg.info, 'error',{ timeout: 1000,closed: function () { $("[name=Address-del]").find("[type=submit]").removeAttr('disabled'); }});
                                }
                            }
                        });
                     },500);
    ...
    $(".switch_status").live("click",function(){
        ...
        var submit = function (v, h, f) {
            if (v == 'ok'){
                ...
                window.setTimeout(function () {
                    $.ajax({
                        url: define_app_url+'/Address/ajax_switch_status',
