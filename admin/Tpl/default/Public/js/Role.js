$(function(){
    ...
    $("#Role-add-edit [title]").live('hover',function(){
        $(this).tooltip({
    ...
    $("#xnfy520-function-index-add").live('click',function(){
        var add_submit = function (v, h, f) {
            if (v == 'ok') {
                ...
                $.ajax({
                    url: define_app_url+'/Role/ajaxinsert',
                    data: f,
                    type:'POST',
        ...
        jBox("get:"+define_app_url+"/Role/add",{
            width:420,
            title: '',
            top: '30%',
            draggable: true,
            ...
            loaded: function (h) {
                var node_type = $("#Role-add-edit [name=level]").val();
                if(node_type==null){
                    jBox.close();
                    jBox.tip("没有可添加用户分组！", 'error',{timeout: 1000});
                }
            },
            submit:add_submit
        });
        ...
        if(value!='' && !isNaN(value)){
            var edit_submit = function (v, h, f) {
                if (v == 'ok') {
                    jBox.tip("正在处理...", 'loading');
                    $.ajax({
                        url: define_app_url+'/Role/ajaxedit',
                        data: f,
                        type:'POST',
                        success: function(data){
                            var msg = JSON.parse(data);
            ...
            jBox("get:"+define_app_url+"/Role/edit/id/"+value,{
                width:420,
                title: '',
                top: '30%',
                draggable: true,
    ...
    //AJAX删除
    $("[name=Role-del]").live('submit',function(){
        ...
        if(data==''){
            ...
            var submit = function (v, h, f) {
                if (v == 'ok'){
                    jBox.tip("正在处理...", 'loading');
                    $("[name=Role-del]").find("[type=submit]").attr('disabled','disabled');
                    window.setTimeout(function () {
                        $.ajax({
                            url: define_app_url+'/Role/ajaxdel',
                            data: data,
                            type:'POST',
                            success: function(data){
                                var msg = JSON.parse(data);
                                if(msg.status==1){
                                    ...
                                    jBox.tip(msg.info, 'error',{ timeout: 1000,closed: function () { $("[name=Role-del]").find("[type=submit]").removeAttr('disabled'); }});
                                }
                            }
                        });
                    },500);
    ...
    $(".switchpublish").click(function(){
        ...
        var submit = function (v, h, f) {
            if (v == 'ok'){
                ...
                window.setTimeout(function () {
                    $.ajax({
                        url: define_app_url+'/Role/ajaxpublish',
