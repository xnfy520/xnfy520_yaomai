$(function(){
    ...
    $("#SystemAnnouncement-add-edit [title]").live('hover',function(){
        $(this).tooltip({
    ...
    $("#xnfy520-function-index-add").live('click',function(){
        var add_submit = function (v, h, f) {
            if (v == 'ok') {
                ...
                $.ajax({
                    url: define_app_url+'/SystemAnnouncement/ajaxinsert',
                    data: f,
                    type:'POST',
        ...
        jBox("get:"+define_app_url+"/SystemAnnouncement/add",{
        ...
        if(value!='' && !isNaN(value)){
            var edit_submit = function (v, h, f) {
                if (v == 'ok') {
                    jBox.tip("正在处理...", 'loading');
                    $.ajax({
                        url: define_app_url+'/SystemAnnouncement/ajaxedit',
                        data: f,
                        type:'POST',
                        success: function(data){
                            var msg = JSON.parse(data);
            ...
            jBox("get:"+define_app_url+"/SystemAnnouncement/edit/id/"+value,{
                width:420,
                title: '',
                top: '20%',
                draggable: true,
    ...
    //AJAX删除
    $("[name=SystemAnnouncement-del]").live('submit',function(){
            ...
            var submit = function (v, h, f) {
                if (v == 'ok'){
                    ...
                    window.setTimeout(function () {
                        $.ajax({
                            url: define_app_url+'/SystemAnnouncement/ajaxdel',
                            data: data,
                            type:'POST',
                            success: function(data){
                                var msg = JSON.parse(data);
    ...
    $(".switch_status").live("click",function(){
        ...
        var submit = function (v, h, f) {
            if (v == 'ok'){
                ...
                window.setTimeout(function () {
                    $.ajax({
                        url: define_app_url+'/SystemAnnouncement/ajax_switch_status',
