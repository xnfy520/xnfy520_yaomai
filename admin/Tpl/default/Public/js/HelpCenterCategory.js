$(function(){
    ...
    $("#HelpCenterCategory-add-edit [title]").live('hover',function(){
        $(this).tooltip({
    ...
    $("#xnfy520-function-index-add").live('click',function(){
        var add_submit = function (v, h, f) {
            if (v == 'ok') {
                ...
                $.ajax({
                    url: define_app_url+'/HelpCenterCategory/ajaxinsert',
                    data: f,
                    type:'POST',
        ...
        jBox("get:"+define_app_url+"/HelpCenterCategory/add",{
            width:423,
            title: '',
            top: '20%',
       //     draggable: true,
        ...
        if(value!='' && !isNaN(value)){
            var edit_submit = function (v, h, f) {
                if (v == 'ok') {
                    jBox.tip("正在处理...", 'loading');
                    $.ajax({
                        url: define_app_url+'/HelpCenterCategory/ajaxedit',
                        data: f,
                        type:'POST',
                        success: function(data){
                            var msg = JSON.parse(data);
            ...
            jBox("get:"+define_app_url+"/HelpCenterCategory/edit/id/"+value,{
                width:420,
                title: '',
                top: '20%',
                draggable: true,
    ...
    //AJAX删除
    $("[name=HelpCenterCategory-del]").live('submit',function(){
            ...
            var submit = function (v, h, f) {
                if (v == 'ok'){
                    ...
                    window.setTimeout(function () {
                        $.ajax({
                            url: define_app_url+'/HelpCenterCategory/ajaxdel',
                            data: data,
                            type:'POST',
                            success: function(data){
                                var msg = JSON.parse(data);
    ...
    $(".switch_status").live("click",function(){
        ...
        var submit = function (v, h, f) {
            if (v == 'ok'){
                ...
                window.setTimeout(function () {
                    $.ajax({
                        url: define_app_url+'/HelpCenterCategory/ajax_switch_status',
