...
//        if(define_cid!=''){
//            window.location=define_app_url+"/VoteCommodity/add/cid/"+define_cid;
//        }else{
//            window.location=define_app_url+"/VoteCommodity/add";
//        }
//    });
    ...
    $(".xnfy520-function-index-edit").live('click',function(){
        ...
        if(id){
            window.location=define_app_url+"/VoteCommodity/edit/id/"+id;
        }else{
            jBox.tip('程序异常', 'error');
        }
    });
    ...
    //AJAX添加
    $("[name=VoteCommodity-add]").live('submit', function() {
        $("[name=VoteCommodity-add]").find("[type=submit]").attr('disabled','disabled');
        var data = $(this).serializeArray();
        var name = $("[name=name]");
        var price = $("[name=price]");
        var subscription = $("[name=subscription]");
        var subscribe = $("[name=subscribe]");
        var expiration_date = $("[name=expiration_date]");
        var image = $("[name=image]");
        var more_image = $("[name='image_more[]']").length;

        if(name.val()===''){
            jBox.tip('商品名称不能为空', 'error',{ timeout: 1000,closed: function () { $("[name=VoteCommodity-add]").find("[type=submit]").removeAttr('disabled'); }});
            return false;
        }
        else if(price.val()==='' || price.val()<=0 || price.val()[0]==0 || isNaN(price.val())){
            jBox.tip('请填写商品售价', 'error',{ timeout: 1000,closed: function () { $("[name=VoteCommodity-add]").find("[type=submit]").removeAttr('disabled'); }});
            return false;
        }
        else if(subscription.val()==='' || subscription.val()<=0 || subscription.val()[0]==0 || isNaN(subscription.val())){
            jBox.tip('请填写商品订金', 'error',{ timeout: 1000,closed: function () { $("[name=VoteCommodity-add]").find("[type=submit]").removeAttr('disabled'); }});
            return false;
        }
        else if(subscribe.val()==='' || subscribe.val()<=0 || subscribe.val()[0]==0 || isNaN(subscribe.val())){
            jBox.tip('请填写预订基本人数', 'error',{ timeout: 1000,closed: function () { $("[name=VoteCommodity-add]").find("[type=submit]").removeAttr('disabled'); }});
            return false;
        }
        else if(expiration_date.val()==''){
            jBox.tip('请选择预订截止日期', 'error',{ timeout: 1000,closed: function () { $("[name=VoteCommodity-add]").find("[type=submit]").removeAttr('disabled'); }});
            return false;
        }
        else if(image.val()===''){
            jBox.tip('商品封面，不能为空', 'error',{ timeout: 1000,closed: function () { $("[name=VoteCommodity-add]").find("[type=submit]").removeAttr('disabled'); }});
            return false;
        }
        else if(more_image<=0){
            jBox.tip('商品图片至少上传一张', 'error',{ timeout: 1000,closed: function () { $("[name=VoteCommodity-add]").find("[type=submit]").removeAttr('disabled'); }});
            return false;
        }
        else{
           jBox.tip("正在处理...", 'loading');
            $.ajax({
                url: define_app_url+'/VoteCommodity/ajax_insert',
                data: data,
                type:'POST',
                success: function(data){
                    ...
                    if(msg.status==1){
                        ...
                        jBox.tip(msg.info, 'error',{ timeout: 1000,closed: function () {
                            $("[name=VoteCommodity-add]").find("[type=submit]").removeAttr('disabled');
                             }});
                    }
                }
            });
    ...
    //AJAX修改
    $("[name=VoteCommodity-edit]").live('submit',function(){
        $("[name=VoteCommodity-edit]").find("[type=submit]").attr('disabled','disabled');
        var data = $(this).serializeArray();
        var id = $("[name=id]");
        var price = $("[name=price]");
        var subscription = $("[name=subscription]");
        ...
        if(id.val()===''){
            jBox.tip('异常操作', 'error',{ timeout: 1000,closed: function () { $("[name=VoteCommodity-edit]").find("[type=submit]").removeAttr('disabled'); }});
            return false;
        }
        else if(price.val()==='' || price.val()<=0 || price.val()[0]==0 || isNaN(price.val())){
            jBox.tip('请填写商品售价', 'error',{ timeout: 1000,closed: function () { $("[name=VoteCommodity-edit]").find("[type=submit]").removeAttr('disabled'); }});
            return false;
        }
        else if(subscription.val()==='' || subscription.val()<=0 || subscription.val()[0]==0 || isNaN(subscription.val())){
            jBox.tip('请填写商品订金', 'error',{ timeout: 1000,closed: function () { $("[name=VoteCommodity-edit]").find("[type=submit]").removeAttr('disabled'); }});
            return false;
        }
        else if(subscribe.val()==='' || subscribe.val()<=0 || subscribe.val()[0]==0 || isNaN(subscribe.val())){
            jBox.tip('请填写预订基本人数', 'error',{ timeout: 1000,closed: function () { $("[name=VoteCommodity-edit]").find("[type=submit]").removeAttr('disabled'); }});
            return false;
        }
        else if(expiration_date.val()==''){
            jBox.tip('请选择预订截止日期', 'error',{ timeout: 1000,closed: function () { $("[name=VoteCommodity-edit]").find("[type=submit]").removeAttr('disabled'); }});
            return false;
        }
        else if(image.val()===''){
            jBox.tip('商品封面，不能为空', 'error',{ timeout: 1000,closed: function () { $("[name=VoteCommodity-edit]").find("[type=submit]").removeAttr('disabled'); }});
            return false;
        }
        else if(more_image<=0){
            jBox.tip('商品图片至少上传一张', 'error',{ timeout: 1000,closed: function () { $("[name=VoteCommodity-edit]").find("[type=submit]").removeAttr('disabled'); }});
            return false;
        }
        else{
            jBox.tip("正在处理...", 'loading');
            $.ajax({
                url: define_app_url+'/VoteCommodity/ajax_edit',
                data: data,
                type:'POST',
                success: function(data){
                    var msg = JSON.parse(data);
                    if(msg.status==1){
                        ...
                        jBox.tip(msg.info, 'error',{ timeout: 1000,closed: function () { $("[name=VoteCommodity-edit]").find("[type=submit]").removeAttr('disabled'); }});
                    }
                }
            });
        }
    ...
    //AJAX删除
    $("[name=VoteCommodity-del]").live('submit',function(){
        var data = $(this).serializeArray();
        if(data==''){
            ...
            var submit = function (v, h, f) {
                if (v == 'ok'){
                    jBox.tip("正在处理...", 'loading');
                    $("[name=VoteCommodity-del]").find("[type=submit]").attr('disabled','disabled');
                    window.setTimeout(function () {
                        $.ajax({
                            url: define_app_url+'/VoteCommodity/ajax_del',
                            data: data,
                            type:'POST',
                            success: function(data){
                                var msg = JSON.parse(data);
                                if(msg.status==1){
                                    ...
                                    jBox.tip(msg.info, 'error',{ timeout: 1000,closed: function () { $("[name=VoteCommodity-del]").find("[type=submit]").removeAttr('disabled'); }});
                                }
                            }
                        });
                    },500);
    ...
    $(".switch_status").live("click",function(){
        ...
        var submit = function (v, h, f) {
            if (v == 'ok'){
                ...
                window.setTimeout(function () {
                    $.ajax({
                        url: define_app_url+'/VoteCommodity/ajax_switch_status',
    ...
    $(".thumb_image").live('click',function(){
        ...
        if(image){
            if(flag==1){
                jBox('<img style="padding: 3px 5px 0 5px;" src="'+define_public_url+'/Content/VoteCommodity/thumb_'+image+'" />',{
    ...
    $(".delete_images_list").live("click",function(){
        ...
        var submit = function (v, h, f) {
            if (v == 'ok'){
                ...
                if(id){
                    $.ajax({
                        url: define_app_url+'/VoteCommodity/delete_image',
                        data: {id:id},
                        type:'POST',
...
/*****/
    ...
    $("#Parameter").click(function(){
        ...
        jBox("get:"+define_app_url+"/VoteCommodity/add_edit_parameter",{
            width:460,
            title: '',
            top: '10%',
            draggable: true,
    ...
    $("#Parameter2").click(function(){
        ...
        jBox("get:"+define_app_url+"/VoteCommodity/add_edit_parameter2",{
            width:460,
            title: '',
            top: '10%',
            draggable: true,
    ...
    $("#Parameter3").click(function(){
        ...
        jBox("get:"+define_app_url+"/VoteCommodity/add_edit_parameter3",{
            width:460,
            title: '',
            top: '10%',
            draggable: true,
    ...
    new AjaxUpload(file_upload_button,{
        ...
        action: define_app_url+'/VoteCommodity/uploads',
        name: 'myfile',
        ...
        onComplete: function(file, response){
            ...
            if(msg.status){
                ...
                if(id){
                    ...
                    $.ajax({
                        url: define_app_url+'/VoteCommodity/add_image',
                        data: {id:id,image:msg.image_name},
                        type:'POST',
                        success: function(datas){
