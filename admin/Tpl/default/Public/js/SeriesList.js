$(function(){
    ...
    $("#SeriesList-add-edit [title]").live('hover',function(){
        $(this).tooltip({
	...
	$("#xnfy520-function-index-add").live('click',function(){
        var add_submit = function (v, h, f) {
            if (v == 'ok') {
                ...
                $.ajax({
                    url: define_app_url+'/SeriesList/ajaxinsert',
                    data: f,
                    type:'POST',
        ...
        jBox("get:"+define_app_url+"/SeriesList/add",{
            width:420,
            title: '',
            top: '30%',
            draggable: true,
        ...
        if(value!='' && !isNaN(value)){
            var edit_submit = function (v, h, f) {
                if (v == 'ok') {
                    jBox.tip("正在处理...", 'loading');
                    $.ajax({
                        url: define_app_url+'/SeriesList/ajaxedit',
                        data: f,
                        type:'POST',
                        success: function(data){
                            var msg = JSON.parse(data);
            ...
            jBox("get:"+define_app_url+"/SeriesList/edit/id/"+value,{
                width:420,
                title: '',
                top: '30%',
                draggable: true,
	...
	//AJAX删除
	$("[name=SeriesList-del]").live('submit',function(){
		...
		if(data==''){
            ...
            var submit = function (v, h, f) {
                if (v == 'ok'){
                    jBox.tip("正在处理...", 'loading');
                    $("[name=SeriesList-del]").find("[type=submit]").attr('disabled','disabled');
                    window.setTimeout(function () {
                        $.ajax({
                            url: define_app_url+'/SeriesList/ajaxdel',
                            data: data,
                            type:'POST',
                            success: function(data){
                                var msg = JSON.parse(data);
                                if(msg.status==1){
                                    ...
                                    jBox.tip(msg.info, 'error',{ timeout: 1000,closed: function () { $("[name=SeriesList-del]").find("[type=submit]").removeAttr('disabled'); }});
                                }
                            }
                        });
                     },500);
	...
	$(".switchpublish").click(function(){
        ...
        window.setTimeout(function () {
            $.ajax({
                url: define_app_url+'/SeriesList/ajaxpublish',
                data: {id:v,publish:k},
                type:'POST',
                success: function(data){
                    var msg = JSON.parse(data);
