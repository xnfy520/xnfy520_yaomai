$(function(){
    ...
    $("[name=pid]").live('change',function(){
        ...
        if(v==0 || v==''){
            ...
            $.ajax({
                url: define_app_url+'/AdvertList/return_tip_size',
                data: {pid:v},
                type:'POST',
                success: function(datas){
                    ...
                    if(msg.status==1){
                        $('.table_image_tip_size').text(msg.data);
                    }else{
    ...
    $("#AdvertList-add-edit [title]").live('hover',function(){
        $(this).tooltip({
    ...
    $("#xnfy520-function-index-add").live('click',function(){
        var add_submit = function (v, h, f) {
            if (v == 'ok') {
                ...
                $.ajax({
                    url: define_app_url+'/AdvertList/ajaxinsert',
                    data: f,
                    type:'POST',
        ...
        jBox("get:"+define_app_url+"/AdvertList/add"+((define_pid) ? '/pid/'+define_pid : ''),{
            width:423,
            title: '',
            top: '20%',
       //     draggable: true,
        ...
        if(value!='' && !isNaN(value)){
            var edit_submit = function (v, h, f) {
                if (v == 'ok') {
                    jBox.tip("正在处理...", 'loading');
                    $.ajax({
                        url: define_app_url+'/AdvertList/ajaxedit',
                        data: f,
                        type:'POST',
                        success: function(data){
                            var msg = JSON.parse(data);
            ...
            jBox("get:"+define_app_url+"/AdvertList/edit/id/"+value,{
                width:420,
                title: '',
                top: '10%',
                draggable: true,
    ...
    //AJAX删除
    $("[name=AdvertList-del]").live('submit',function(){
            ...
            var submit = function (v, h, f) {
                if (v == 'ok'){
                    ...
                    window.setTimeout(function () {
                        $.ajax({
                            url: define_app_url+'/AdvertList/ajaxdel',
                            data: data,
                            type:'POST',
                            success: function(data){
                                var msg = JSON.parse(data);
    ...
    $(".switch_status").live("click",function(){
        ...
        var submit = function (v, h, f) {
            if (v == 'ok'){
                ...
                window.setTimeout(function () {
                    $.ajax({
                        url: define_app_url+'/AdvertList/ajax_switch_status',
