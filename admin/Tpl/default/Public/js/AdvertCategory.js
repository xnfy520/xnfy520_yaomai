$(function(){
    ...
    $("#AdvertCategory-add-edit [title]").live('hover',function(){
        $(this).tooltip({
    ...
    $("#xnfy520-function-index-add").live('click',function(){
        var add_submit = function (v, h, f) {
            if (v == 'ok') {
                ...
                $.ajax({
                    url: define_app_url+'/AdvertCategory/ajaxinsert',
                    data: f,
                    type:'POST',
        ...
        jBox("get:"+define_app_url+"/AdvertCategory/add",{
            width:423,
            title: '',
            top: '20%',
       //     draggable: true,
            ...
            loaded: function (h) {
                var advert_type = $("#AdvertCategory-add-edit [name=type]").val();
                if(advert_type==null){
                    jBox.close();
                    jBox.tip("没有可添加广告分类！", 'error',{timeout: 1000});
                }
            },
            submit:add_submit
        });
        ...
        if(value!='' && !isNaN(value)){
            var edit_submit = function (v, h, f) {
                if (v == 'ok') {
                    jBox.tip("正在处理...", 'loading');
                    $.ajax({
                        url: define_app_url+'/AdvertCategory/ajaxedit',
                        data: f,
                        type:'POST',
                        success: function(data){
                            var msg = JSON.parse(data);
            ...
            jBox("get:"+define_app_url+"/AdvertCategory/edit/id/"+value,{
                width:420,
                title: '',
                top: '20%',
                draggable: true,
    ...
    //AJAX删除
    $("[name=AdvertCategory-del]").live('submit',function(){
            ...
            var submit = function (v, h, f) {
                if (v == 'ok'){
                    ...
                    window.setTimeout(function () {
                        $.ajax({
                            url: define_app_url+'/AdvertCategory/ajaxdel',
                            data: data,
                            type:'POST',
                            success: function(data){
                                var msg = JSON.parse(data);
    ...
    $(".switch_status").live("click",function(){
        ...
        var submit = function (v, h, f) {
            if (v == 'ok'){
                ...
                window.setTimeout(function () {
                    $.ajax({
                        url: define_app_url+'/AdvertCategory/ajax_switch_status',
