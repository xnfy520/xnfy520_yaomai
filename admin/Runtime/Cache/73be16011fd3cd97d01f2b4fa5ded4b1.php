...
<div id="GrouponCommodity-add-edit">
    ...
    <form action="javascript:;" method="POST" name="GrouponCommodity-edit">
        ...
        <ul style="margin-top: 10px;">
            ...
            <li class="xnfy520-function-index-add-edit-ul-li" style="margin-top: 2px;">
                ...
                <select name="pid" style="padding: 2px;margin-left: 0;min-width:50px">
                    <?php if(is_array($clist)): $i = 0; $__LIST__ = $clist;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$volist): $mod = ($i % 2 );++$i; if(($volist["id"]) == $data["pid"]): ?><option selected="selected" value="<?php echo ($volist["id"]); ?>"><?php echo ($volist["name"]); ?></option>
                            <?php else: ?>
            ...
            <li class="xnfy520-function-index-add-edit-ul-li" style="margin-top: 3px;">
                <strong>原　　价：</strong>
                <input type="text" name="org_price" value="<?php echo (($data["org_price"])?($data["org_price"]):0.00); ?>" style="width: 48px;text-align: center;height: 18px;" maxlength="8" />　
                <strong>现　　价：</strong>
                    <input type="text" name="price" value="<?php echo (($data["price"])?($data["price"]):0.00); ?>" style="width: 47px;text-align: center;height: 18px;" maxlength="8" />　
                <strong>截止日期：</strong>
                <input type="text" name="expiration_date" value="<?php echo (date("Y-m-d",($data["expiration_date"])?($data["expiration_date"]):'')); ?>" class="Wdate" type="text" onfocus="WdatePicker({minDate:'%y-%M-{%d+1}'})" readonly="readonly" style="width: 100px;height: 18px;text-indent:5px" />　
                <strong>排　　序：</strong>
            ...
            <li class="xnfy520-function-index-add-edit-ul-li" style="margin-top: 5px;height: 50px;background: #d6fffa;padding: 5px 0;">
                  <span class="positioning_images" style="float: left;position: relative;margin-left: 8px;width: 50px;height: 50px;display: block;padding-right: 16px;border-right: 1px dashed blue;">
                        <img class="thumb_image"
                             ...
                             src="__PUBLIC__/Content/GrouponCommodity/cut_<?php echo ($data["image"]); ?>" />
            ...
            <li class="texli" style="margin-bottom: 10px;">
                 <textarea class="details" name="details" style="resize: none;position:absolute;visibility: hidden;float: left;"><?php echo ($data["details"]); ?></textarea>
            </li>
            <li>
