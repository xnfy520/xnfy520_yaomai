...
<script type="text/javascript">
    $(function(){
        $(".znxx_m td").css({'border':'none'});
        $(".znxx_m td span").css({'position':'relative','top':'-8px','margin-left':'20px','font-weight':'bold','margin-right':'5px'});
    });
</script>
    ...
    <div id="xnfy520-function-index-content" style="max-height:460px;overflow-y:scroll;">
        ...
        <form action="javascript:;" method="post" name="OrderList-del" class="delete">
            <table id="xnfy520-function-index-table">
                ...
                <?php else: ?>
                    <?php if(is_array($list)): $i = 0; $__LIST__ = $list;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($i % 2 );++$i;?><tr class="znxx">
                            ...
                            <td style="text-indent:5px;<?php if(($_GET['type']) == "4"): ?>text-align:center;<?php else: ?>text-align:left;<?php endif; ?>">
                                <span style="min-width:80px;display:inline-block">
                                    <?php if(($vo["new_price"]) > "0"): ?><s>&#165;<?php echo ($vo["total_fee"]); ?></s>　
                                        <span style="color:red">&#165;<?php echo ($vo["new_price"]); ?></span>　
                                    <?php else: ?>
                                        &#165;<?php echo ($vo["total_fee"]); endif; ?>
                                </span>
                                <?php if(($_GET['type']) != "4"): ?>(
                                含物流:&#165;<?php echo ($vo["other_data"]["logistics"]); ?>
                                <?php if(!empty($vo["other_data"]["coupon"])): ?>已优惠:-<?php echo ($vo["other_data"]["coupon"]); endif; ?>
                                )<?php endif; ?>
                            </td>
                            <td>
                            ...
                            <td>
                               <?php if(($_GET['type']) == "4"): if(($vo["abolish"]) == "1"): if(($vo["abolish_dispose"]) == "1"): ?><span by="<?php echo ($vo["id"]); ?>" type="abolish_dispose" value="<?php echo ($vo["abolish_dispose"]); ?>" class="abolish_dispose" style="color:blue;font-weight:bold;cursor:pointer">已处理</span>
                                    ...
                                    <?php else: ?>
                                        <span class="change_price" by="<?php echo ($vo["id"]); ?>" style="color:#999;font-weight:bold;cursor:pointer">改价</span><?php endif; ?> 
                               <?php else: ?>
                                    <?php if(($vo["pay_type"]) == "0"): ?><span class="change_price" by="<?php echo ($vo["id"]); ?>" style="color:#999;font-weight:bold;cursor:pointer">改价</span>
                                    <?php else: ?>
                                ...
                                <table width="100%" cellpadding="0" cellspacing="0" class="tab0001">
                                        ...
                                        <?php if(is_array($vo["commodity_data"])): $i = 0; $__LIST__ = $vo["commodity_data"];if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo_mo_ci): $mod = ($i % 2 );++$i;?><tr style="line-height: 0;">
                                                <td colspan="7" style="padding-top:0; border:none">
                                                    <div class="znxx_m" style="width:100%; padding:0;">
                                                        <table width="100%" cellpadding="0" cellspacing="0" style="border-collapse: collapse;border:none">
                                                            <tr style="border:none;">
                                                                <td width="200" align="left" style="border: none;">
                                                                    <span style="margin-top:5px">#<?php echo ($vo_mo_ci["Commodity"]["id"]); ?></span>
                                    ...
                                    <tr>
                                        <td colspan="7" style="border: none;text-align:left;text-indent:20px">
                                            <?php if(($_GET['type']) == "4"): ?><strong>商品编号</strong>：#<?php echo ($vo["commodity_data"]["id"]); ?>　<?php endif; ?>
                                            <strong>支付方式</strong>：
                                            ...
                                            <?php else: ?>
                                                ...
                                                <?php if(($vo["other_data"]["payment_model"]) == "1"): ?><strong>支付平台</strong>：
                                                    <?php switch($vo["other_data"]["payment"]): case "1": ?>支付宝 ( 支付帐号 <?php echo (($vo["alipay_data"]["buyer_email"])?($vo["alipay_data"]["buyer_email"]):"-"); ?> )　
                                                        <strong>支付时间</strong>: <?php if(empty($vo["alipay_data"]["pay_date"])): ?>-<?php else: echo (date("Y-m-d H:i:s",$vo["alipay_data"]["pay_date"])); endif; break;?>
                                                        <?php case "2": ?>网上银行<?php break; endswitch; endif; ?>　<?php endif; ?>
                                        </td>
                                    </tr>
                                    <?php if(($_GET['type']) != "4"): ?><tr>
                                            <td colspan="7" style="border: none;text-align:left;text-indent:20px">
                                                <strong>收货人姓名</strong>：<?php echo ($vo["address"]["consignee"]); ?>　
                                    ...
                                    <tr>
                                        <td colspan="7" style="border: none;text-align:left;text-indent:20px">
                                            <strong>订单备注</strong>：
                                            <?php if(empty($vo["remark"])): ?>-<?php else: echo ($vo["remark"]); endif; ?>　<span class="edit_remark" by="<?php echo ($vo["id"]); ?>" style="color:red;font-weight:bold;cursor:pointer">编辑</span>
                                        </td>
                                    </tr>
                                </table>
                            </td>
            ...
            <?php if(!empty($list)): ?><div style="border:1px solid #ccc;height: 37px;">
                    ...
                    <?php switch($_GET['pid']): case "1": ?>昨日<?php break;?>
                        <?php case "2": ?>今日<?php break;?>
                        <?php case "3": ?>本周<?php break;?>
                        <?php case "4": ?>上周<?php break;?>
                        <?php case "5": ?>本月<?php break;?>
                        <?php case "6": ?>上月<?php break;?>
                        <?php default: endswitch;?>总计:&#165;<span id="totals_prices" t="<?php echo ($total_price); ?>">-</span>
                    </div>
        ...
        <script>
            $(function(){
                ...
                $("#totals_prices").html(parseFloat($("#totals_prices").attr('t')).toFixed(2));
            })
        </script>
    </div>
</div>
