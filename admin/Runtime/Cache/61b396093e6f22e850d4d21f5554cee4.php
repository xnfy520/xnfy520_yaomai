...
<div id="OtherLinks-add-edit">
    ...
    <table>
        <caption>付款状态</caption>
        <tr><th class="table_th table_th_adjust"><span class="table_star">*</span> 确认付款：</th>
            <td class="table_td">
                <select name="pay_type" class="table_select">
                    <?php if(($data["pay_type"]) == "0"): ?><option value="0" selected="selected">未付款</option>
                        <option value="3">已付款</option>
                    <?php else: ?>
                        <option value="0">未付款</option>
                        <option value="3" selected="selected">已付款</option><?php endif; ?>
                </select>
            </td>
        </tr>
        <tr><th class="table_th_top">备注信息：</th><td><textarea class="textarea" name="remarks" title="限1-255字符 (可为空)"><?php echo ($data["manual_data"]["remarks"]); ?></textarea></td></tr>
        <?php if(($data["pay_type"]) == "3"): if(!empty($data["manual_data"]["update_date"])): ?><tr><th class="table_th_top">付款时间：</th><td><?php echo (date("Y-m-d H:i:s",$data["manual_data"]["update_date"])); ?></td></tr><?php endif; endif; ?>
    </table>
</div>
