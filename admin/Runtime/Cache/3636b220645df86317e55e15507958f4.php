...
<div id="AdvertList-add-edit">
    <input type="hidden" name="id" value="<?php echo ($data["id"]); ?>" />
    <table>
        <caption>修改广告</caption>
        <tr class="table_tr">
        ...
        <tr><th class="table_th table_th_adjust"><span class="table_star">*</span> 所属分类：</th>
            <td class="table_td">
                <select name="pid" class="table_select">
                    <?php if(is_array($clist)): $i = 0; $__LIST__ = $clist;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($i % 2 );++$i; if(($vo["id"]) == $data["pid"]): ?><option value="<?php echo ($vo["id"]); ?>" selected="selected"><?php echo ($vo["name"]); ?></option>
                            <?php else: ?>
        ...
        <tr>
            ...
            <td class="table_td">
                ...
                <span class="table_image_tip_size"><?php echo ($tip_size); ?></span>
            </td>
        </tr>
        ...
        <tr <?php if(empty($data["image"])): ?>style="display:none;"<?php endif; ?> class="thumb_image_tr"><th class="table_th"></th>
            <td>
                <span class="positioning_image">
                    <img class="thumb_image" src="__PUBLIC__/Content/AdvertList/thumb_<?php echo ($data["image"]); ?>">
...
<script>
    $(function(){
        ...
        new AjaxUpload(file_upload_button,{
            action: define_app_url+'/AdvertList/uploads',
            name: 'myfile',
