...
<div id="CommodityCategory-add-edit">
    ...
    <table>
        ...
        <tr class="table_tr" style="display: none;"><th class="table_th"><span class="table_star">*</span> 名　　称：</th><td class="table_td"><input type="text" name="name" value="<?php echo ($data["name"]); ?>" maxlength="50" title="限1-50字符 (必填)" /></td></tr>
        <tr><th class="table_th table_th_adjust"><span class="table_star">*</span> 名　　称：</th>
            <td class="table_td">
                <select name="function" class="table_select">
                    <?php if(is_array($Commodity_Category)): $i = 0; $__LIST__ = $Commodity_Category;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($i % 2 );++$i; if(($vo["function"]) == $data["function"]): ?><option value="<?php echo ($vo["function"]); ?>" selected="selected"><?php echo ($vo["name"]); ?></option>
                        <?php else: ?>
                            <option value="<?php echo ($vo["function"]); ?>"><?php echo ($vo["name"]); ?></option><?php endif; endforeach; endif; else: echo "" ;endif; ?>
                </select>
            </td>
        </tr>
        ...
        <tr>
            <th class="table_th table_th_adjust"><span class="table_star">*</span> 图　　片：
            </th>
            <td class="table_td">
        ...
        <tr <?php if(empty($data["image"])): ?>style="display:none;"<?php endif; ?> class="thumb_image_tr"><th class="table_th"></th>
            <td>
                <span class="positioning_image">
                    <img class="thumb_image" src="__PUBLIC__/Content/CommodityCategory/thumb_<?php echo ($data["image"]); ?>">
        ...
        <tr class="table_tr"><th class="table_th">图片链接：</th><td class="table_td"><input type="text" name="link" value="<?php echo ($data["link"]); ?>" title="必须以 http:// 开头 双击可显示 (可为空)" /></td></tr>
