...
<div id="xnfy520-function-index">
    ...
    <div id="xnfy520-function-index-title">
        <?php echo ($Role_name["name"]); ?> 列表 <button type="button" class="send_message" message_type="<?php echo ($instation_message_type["group"]); ?>"  addressee_id="<?php echo ($Role_name["id"]); ?>" messge_title="所有 <?php echo ($Role_name["name"]); ?>">群发站内信</button>
    </div>

    <!--<div id="xnfy520-function-index-nav">-->
        <!--<a href="__APP__/User/index">全部</a>-->
        <!--<?php if(is_array($clist)): $i = 0; $__LIST__ = $clist;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$voc): $mod = ($i % 2 );++$i;?>-->
            <!--<a href="__APP__/User/index/rid/<?php echo ($voc["id"]); ?>"><?php echo ($voc["name"]); ?></a>-->
    ...
    <div id="xnfy520-function-index-search">
        ...
        <form action="__APP__/User/index<?php if(!empty($_GET['rid'])): ?>/rid/<?php echo ($_GET['rid']); endif; ?>" method="post" name="form_search"  style="line-height: 35px;">
    ...
    <div id="xnfy520-function-index-content">
        ...
        <form action="javascript:;" method="post" name="User-del" class="delete">
            <table id="xnfy520-function-index-table">
                <thead>
                <tr>
                    <th id="xnfy520-function-index-table-th-name">名称</th>
                    <th id="xnfy520-function-index-table-th-email">邮箱</th>
                    <th class="xnfy520-function-index-table-th-50">站内信</th>
                    <!--<th class="xnfy520-function-index-table-th-50">权限</th>-->
                    <th class="xnfy520-function-index-table-th-50">启用</th>
                    <!--<th class="xnfy520-function-index-table-th-50">审核</th>-->
                    <th class="xnfy520-function-index-table-th-100">会员中心</th>
                    <th id="xnfy520-function-index-table-th-check-all">
                        <?php if(!empty($list)): ?><input type="button" value="全选" /><?php endif; ?>
                    </th>
                </tr>
                </thead>
                <tbody>
                <?php if(empty($list)): ?><tr><td colspan="5" style="height:50px;">暂无数据</td></tr>
                    ...
                    <?php if(is_array($list)): $i = 0; $__LIST__ = $list;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($i % 2 );++$i;?><tr>
                            <td class="xnfy520-function-index-table-td-name">
                                   <span class="xnfy520-function-index-edit" id="index-edit-name">
                                       <?php if((mb_strlen($vo["username"],'utf-8')) > "20"): ?><span><?php echo (mb_substr($vo["username"],0,20,"utf-8")); ?>...</span>
                                           <?php else: ?>
                                           <span><?php echo ($vo["username"]); ?></span><?php endif; ?>
                                    </span>
                            </td>
                            <td class="xnfy520-function-index-table-td-email">
                                   <span class="xnfy520-function-index-edit">
                                       <?php if((mb_strlen($vo["email"],'utf-8')) > "20"): ?><span><?php echo (mb_substr($vo["email"],0,20,"utf-8")); ?>...</span>
                                           <?php else: ?>
                                           <span><?php echo ($vo["email"]); ?></span><?php endif; ?>
                                    </span>
                            </td>
                            <td>
                                <button type="button" class="send_message" message_type="<?php echo ($instation_message_type["personal"]); ?>" addressee_id="<?php echo ($vo["id"]); ?>" messge_title="<?php echo ($vo["username"]); ?>">发送</button>
                            </td>
                            <!--<td>-->
                                <!--&lt;!&ndash;<button type="button" class="Set-Permission" roleid="<?php echo ($vo["roleid"]); ?>">设置</button>&ndash;&gt;-->
                            <!--</td>-->
                            <td class="xnfy520-function-index-table-td-enable">
                                <?php if(($vo["id"]) == "1"): ?>-
                                    <?php else: ?>
                                    <?php if(($vo["status"]) == "1"): ?><span class="switch_status" by="<?php echo ($vo["id"]); ?>" type="status" value="<?php echo ($vo["status"]); ?>">是</span>
                                        <?php else: ?>
                                        <span class="switch_status" by="<?php echo ($vo["id"]); ?>" type="status" value="<?php echo ($vo["status"]); ?>">否</span><?php endif; endif; ?>
                            ...
                            <!--<td class="xnfy520-function-index-table-td-enable">-->
                                <!--<?php if(($vo["id"]) == "1"): ?>-->
                                    ...
                                    <!--<?php else: ?>-->
                                    <!--<?php if(($vo["audit"]) == "1"): ?>-->
                                        <!--<span class="switch_status" by="<?php echo ($vo["id"]); ?>" type="audit" value="<?php echo ($vo["audit"]); ?>">是</span>-->
                                        <!--<?php else: ?>-->
                                        <!--<span class="switch_status" by="<?php echo ($vo["id"]); ?>" type="audit" value="<?php echo ($vo["audit"]); ?>">否</span>-->
                                    <!--<?php endif; ?>-->
                                <!--<?php endif; ?>-->

                            <!--</td>-->
                            <!--<td><?php echo (date("Y-m-d H:i:s",$vo["regdate"])); ?></td>-->
                            <td><span style="cursor: pointer;" class="into-member" by_user="<?php echo ($vo["id"]); ?>" >个人中心</span></td>
                            <td class="xnfy520-function-index-table-td-checkbox">
                                <?php if(($vo["id"]) == "1"): ?><input style="display: none;" class="xnfy520-function-index-table-check-this" disabled="disabled" type="checkbox" name="deleteid[]" value="<?php echo ($vo["id"]); ?>" />
                                    <?php else: ?>
                                        <input class="xnfy520-function-index-table-check-this" type="checkbox" name="deleteid[]" value="<?php echo ($vo["id"]); ?>" /><?php endif; ?>
                            </td>
