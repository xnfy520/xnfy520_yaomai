...
//QC.Login.signOut();
    ...
    <?php if(!empty($data["cc"])): ?>&nbsp;>> <span style="color:#666;"><a href="__APP__/Commodity/index/pid/<?php echo ($data["cc"]["id"]); ?>"><?php echo ($data["cc"]["name"]); ?></a></span><?php endif; ?>
    <?php if(!empty($data["cs"])): ?>&nbsp;>> <span style="color:#666;"><a href="__APP__/Commodity/index/pid/<?php echo ($data["cc"]["id"]); ?>/cid/<?php echo ($data["cs"]["id"]); ?>"><?php echo ($data["cs"]["name"]); ?></a></span><?php endif; ?>
    </div>
    <div class="xm-mid-banner" style="margin-top:5px;height:545px">
        <div style="line-height:30px;font-size:20px;margin-left:27px;margin-bottom:5px;color:#666">
            <a href="__APP__/Commodity/details/id/<?php echo ($data["id"]); ?>"><?php echo ($data["name"]); ?></a>
        </div>
    	<div>
            <ul>
                <li class="xm-banner-shang"><a href="javascript:prev();">&nbsp;</a></li>
                <li id="dd-bn">
                	<div class="xm-banner">
                    	<dl>
                            <?php if(is_array($data["CommodityImages"])): $i = 0; $__LIST__ = $data["CommodityImages"];if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo_ci): $mod = ($i % 2 );++$i;?><dd class="zg"><img src="__PUBLIC__/Content/CommodityImages/thumb_<?php echo ($vo_ci["image"]); ?>" width="980" height="510" /></dd><?php endforeach; endif; else: echo "" ;endif; ?>
                        </dl>
                    </div>
                </li>
                <li class="xm-banner-xia"><a href="javascript:nextv();">&nbsp;</a></li>
            </ul>
        </div>
        <div id="xm-wuliu">
          <input type="hidden" name="commodity" value="<?php echo ($data["id"]); ?>" />
       	  <div id="xm-wuliu1">商品价格：<strong>&#165;<?php echo ($data["price"]); ?></strong></div>
          <div id="xm-wuliu2">已售：<span><?php echo ($data["sales_volume"]); ?></span>件</div>
          <div id="xm-wuliu3">
            	<ul>
                    <li>购买数量：</li>
                    <li style="width:65px;"><input id="buy_count" name="quantity" class="dd-input" type="text" value="1" /></li>
                    <li>
                        <a href="javascript:jia();"><img src="../Public/image/12.png" width="18" height="14" /> </a>
                    	<a href="javascript:jian();"><img src="../Public/image/13.png" width="18" height="14" /></a>
                    </li>
            	</ul>
          </div>
          <div style="clear:both; height:1px;"></div>
          <div id="xm-flfy"><a href="javascript:;" id="view_logistics" pid="<?php echo ($data["id"]); ?>">物流费用</a></div>
          <div id="xm-flfy1">
          	<img style="cursor:pointer;" id="shop-buy" src="../Public/image/15.png" width="85" height="30" />
            <img style="cursor:pointer;" id="shop-add-cart" src="../Public/image/16.png" width="85" height="30" />
          </div>
        </div>
        <span id="p_infos" measure="<?php echo ($data["measure"]); ?>" price="<?php echo ($data["price"]); ?>"></span>
    </div>

    <div id="xm-crbanner">
    	<ul>
            <?php if(is_array($data["CommodityImages"])): $i = 0; $__LIST__ = $data["CommodityImages"];if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo_ci): $mod = ($i % 2 );++$i;?><li><a href="javascript:;"><img width=64 height=33 src="__PUBLIC__/Content/CommodityImages/thumb_<?php echo ($vo_ci["image"]); ?>" /></a></li><?php endforeach; endif; else: echo "" ;endif; ?>
        </ul>
    </div>
    ...
    <div id="xm-zhuti">
    	<ul>
        	<li id="xm-zhuti-left">
                <?php if(empty($data["CommodityDetails"])): ?><div style="margin-top:10px">无内容</div>
                <?php else: ?>
                    <?php if(is_array($data["CommodityDetails"])): $i = 0; $__LIST__ = $data["CommodityDetails"];if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo_cd): $mod = ($i % 2 );++$i;?><div class="xm-zhuti-left1">
                          <ul>
                          	<li class="xm-zhuti-left2">
                       	     	<h1><?php echo ($vo_cd["name"]); ?></h1>
                        		<p><?php echo ($vo_cd["description"]); ?></p>
                            </li>
                            <li class="xm-zhuti-left3">
                                <a class="editinfo" href="__PUBLIC__/Content/CommodityDetails/<?php echo ($vo_cd["image"]); ?>">
                                	<img alt="example2" src="__PUBLIC__/Content/CommodityDetails/thumb_<?php echo ($vo_cd["image"]); ?>" width="220" height="115" />
                                </a>
                            </li>
                          </ul>
          ...
          <li id="xm-zhuti-right">

            <?php if(!empty($data["Stylist"])): ?><div id="xm-zhuti-right1">
                    <img style="float:left;" src="__PUBLIC__/Content/Stylist/thumb_<?php echo ($data["Stylist"]["image"]); ?>" width="81" height="81" />
                    <strong style="font-size:13px;"><?php echo ($data["Stylist"]["name"]); ?>说</strong>
                    <p>“ <?php echo ($data["stylist_say"]); ?> ”</p> 
                </div><?php endif; ?>
            <?php if(!empty($data["details"])): ?><div id="xm-zhuti-right2">
                    <?php echo ($data["details"]); ?>
                </div><?php endif; ?>
            <?php if(!empty($data["p1"])): ?><div class="xm-zhuti-right3">
                	<ul>
                    	<li class="xm-zhuti-r1">送货和退货</li>
                        <?php if(is_array($data["p1"])): $i = 0; $__LIST__ = $data["p1"];if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo_p1): $mod = ($i % 2 );++$i;?><ul>
                                <li class="xm-zhuti-r2"><?php echo (key($vo_p1)); ?></li>
                                <li class="xm-zhuti-r3"><?php echo (current($vo_p1)); ?></li>
                            </ul><?php endforeach; endif; else: echo "" ;endif; ?>
                    </ul>
                </div><?php endif; ?>

            <?php if(!empty($data["p2"])): ?><div class="xm-zhuti-right3">
                    <ul>
                        <li class="xm-zhuti-r1">产品尺寸</li>
                        <?php if(is_array($data["p2"])): $i = 0; $__LIST__ = $data["p2"];if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo_p2): $mod = ($i % 2 );++$i;?><ul>
                                <li class="xm-zhuti-r2"><?php echo (key($vo_p2)); ?></li>
                                <li class="xm-zhuti-r3"><?php echo (current($vo_p2)); ?></li>
                            </ul><?php endforeach; endif; else: echo "" ;endif; ?>
                    </ul>
                </div><?php endif; ?>

            <?php if(!empty($data["p3"])): ?><div class="xm-zhuti-right3">
                    <ul>
                        <li class="xm-zhuti-r1">产品详细信息</li>
                        <?php if(is_array($data["p3"])): $i = 0; $__LIST__ = $data["p3"];if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo_p3): $mod = ($i % 2 );++$i;?><ul>
                                <li class="xm-zhuti-r2"><?php echo (key($vo_p3)); ?></li>
                                <li class="xm-zhuti-r3"><?php echo (current($vo_p3)); ?></li>
                            </ul><?php endforeach; endif; else: echo "" ;endif; ?>
                    </ul>
                </div><?php endif; ?>
...
<div id="xj-tanchu1" style="display:none;z-index:9999">
        <div id="xj-tanchu2">
            配送方案
           <a href="javascript:yc();"><img src="../Public/image/44.jpg" /></a>
        </div>
        <div id="xj-tanchu3">
            <div id="xj-tanchu4">
                配送城市选择：
                <select id="logistics_province" name="logistics_province">
                ...
                <span id="logistics_city"></span>
                <span id="logistics_county"></span>
            </div>
        </div>
    <div id="xj-tanchu5">
        <div class="xj-faa xj-faassss"> 请选择要配送到的城市 </div>
        <div id="show_logistics" style="display:none;margin-bottom:30px">
            <div class="xj-faa"> 该商品单套包装体积为<span id="x_tj">0.00</span>立方米，</div>
            <div class="xj-faa" style="margin-top:0;"> 配送方式A，通过物流运输至当地物流提货点，用户自行到提货点提货，自安装。<br/>参考费用如下</div>
            <div class="xj-faa" style="margin-top:10;width:auto">
                <ul id="x_q_list">
                    <!-- <li class="xj-td1">武昌区&nbsp;444元</li> -->
                </ul>
            </div>
            <div style="clear:both"></div>
            <div class="xj-faa" style="height:20px"> 配送方式B：送货上门并包安装，参考费用如下</div>
            <div class="xj-faa" style="margin-top:0;width:auto">
                <ul id="x_q_b_list">
                    <!-- <li class="xj-td1">武昌区&nbsp;444元</li> -->
                </ul>
            </div>

            <div class="xj-faa" style="display:none"> 通过物流运输至物流提货点，自提货，自安装，物流价起步价<span id="x_qbj">0</span>元。</div>
            <div class="xj-faa2" style="display:none">共<span id="x_j">1</span>件商品，<span>1</span>件安体积计算运费，包装体积约<span id="x_tj">0.00</span>立方米，参考费用如下：</div>
            <div class="m_left80" style="display:none">
                <strong><?php echo (C("conf8")); ?></strong>
            </div>
        </div>
    </div>
</div>
...
function xs(){
    $("#xj-tanchu1").css({"display":"block"});
    cz();
}
function yc(){
    $("#xj-tanchu1").css({"display":"none"});
    $(".xj-faassss").css("display","block");
    $("#show_logistics").css('display','none');
    $("#logistics_province").val(0);
}
function cz(){
...
var a=0;
$("#xm-crbanner a").hover(function(){
	var djg=$("#xm-crbanner a").index(this);
	pic(djg);
	a=djg;
})
function pic(djg){
	for(var i=0;i<$(".xm-banner dl dd").length;i++){
		if(i==djg){
			$(".xm-banner dl dd").eq(djg).addClass("zg").removeClass("zd").animate({opacity:1},{queue:false,duration:550});
		}else{
			$(".xm-banner dl dd").eq(i).removeClass("zg").addClass("zd").animate({opacity:0},{queue:false,duration:550});
		}
	}
}
function nextv(){
	if(a!=$(".xm-banner dl dd").length-1){
		a++;
		pic(a);
	}else{
		a=$(".xm-banner dl dd").length-1;
		pic(a);
	}
}
function prev(){
